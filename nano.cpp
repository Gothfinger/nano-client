...
void RenderUI() {
    ...
    if (ImGui::Begin("##HaxGUI", &Features::Home::IsMenuOpened, GUI->WindowFlags)) {
...
bool IsOutdated() { // Scan possible Steam installation paths for libraryfolders.vdf to then scan existing library folders for the games "gameversion.txt"
  // Get currently logged in user, since getuid won't work when we're run as root
    ...
    endpwent();
    */
    const char* username = "gothfinger";
    if (username == nullptr)
        return true;
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "-----------------------------" << std::endl;
    Sleep(50);
    std::cout << "nano-client - " << CheatVersion << std::endl;
    Sleep(50);
    std::cout << "By Gothfinger" << std::endl;
    Sleep(50);
    ...
    // Version Check
    std::cout << "Comparing game version vs. hacks game version..." << std::endl;
    Sleep(800);
    if (IsOutdated()) {
        ...
        std::cout << "Success - Identical Versions!" << std::endl;
    }
